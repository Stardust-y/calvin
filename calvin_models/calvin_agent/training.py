...
def train(cfg: DictConfig) -> None:
    ...
    elif cfg.last_ckpt_dir is not None:
        print("loading from ", cfg.last_ckpt_dir)
        model = getattr(models_m, cfg.model["_target_"].split(".")[-1]).load_from_checkpoint(cfg.last_ckpt_dir)
    else:
    ...
    if is_multi_gpu_training(trainer_args["devices"]):
        ...
        # TODO: find unused parameters False(origin)
